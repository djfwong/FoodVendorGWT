                ...
                new AsyncCallback<List<VendorFeedback>>() {
                    ...
                    public void onSuccess(List<VendorFeedback> result) {
                        if (result != null) {
                            ...
                            mean /= result.size();
                            String formatted = NumberFormat.getFormat("0.0")
                                    .format(mean);
                            stats1.add(new HTMLPanel("span", ", "
                                    + result.size() + " reviews, " + formatted
                                    + " average rating"));
                        }
                    }
                });
    ...
    private Anchor getEditButton(final VerifiedVendor v) {
        ...
        editLink.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                messages.clear();
                modal[0].add(messages);

                // The form
                HTMLPanel form = new HTMLPanel("form", "");
                HTMLPanel fieldSet = new HTMLPanel("fieldset",
        ...
        submitButton.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                if (!phoneNo.getText().isEmpty()
                        && !FormValidator.validatePhoneNo(phoneNo.getText())) {
                    messages.insert(getMessageWidget(true,
                            "Please ensure your phone number contains 10 numbers and no invalid characters."),
                            0);
                } else if (!email.getText().isEmpty()
                        && !FormValidator.validateEmail(email.getText())) {
                    messages.insert(getMessageWidget(true,
                            "Please enter a valid email address."), 0);
                } else if (!hours.getText().isEmpty()
                        && FormValidator.containsIllegal(hours.getText())) {
                    messages.insert(getMessageWidget(true,
                            "Please remove any invalid characters from the hours text box."), 0);
                } else if (!deals.getText().isEmpty()
                        && FormValidator.containsIllegal(deals.getText())) {
                    messages.insert(getMessageWidget(true,
                            "Please remove any invalid characters from the hours text box."), 0);
                } else {
                    VerifiedVendorServiceAsync verifiedVendorService = GWT
                            ...
                            new AsyncCallback<Boolean>() {
                                ...
                                public void onSuccess(Boolean result) {
                                    module.addMessage(false,
                                            "Updating vendor was successful!");
                                    modal[0].hide();
                                }
                            });
                }
    ...
    private Anchor getDeleteButton(final VerifiedVendor v) {
        ...
        deleteLink.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                final Modal modal = module.addModal("Confirm Deletion",
                        areYouSure);
                ...
                confirm.addClickHandler(new ClickHandler() {
                    ...
                    public void onClick(ClickEvent event) {
                        VerifiedVendorServiceAsync verifiedVendorService = GWT
                                .create(VerifiedVendorService.class);
                        verifiedVendorService.removeVerifiedVendor(
                                v.getVendorId(), new AsyncCallback<Boolean>() {
                            @Override
                            public void onFailure(Throwable caught) {
                                module.addMessage(true,
