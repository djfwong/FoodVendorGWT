...
public class FavouritesTab extends AbstractNavbarTab {
	...
	public FlowPanel getContent()
	{
		...
		final HTMLPanel info = new HTMLPanel(
				"<p class=\"lead\">All your favourite food vendors are listed below! You may also remove any of your saved favourites. Note: if you choose to remove a saved vendor, notice the changes will not be reflected right away on this page.</p>");
		favourites.add(info);
		...
		favouritesService.getUserFavourites(user.getId(),
			...
			public void onSuccess(List<Favourite> result)
			{
				...
				else
				{
					...
					HTMLPanel header = new HTMLPanel("p", "You have "
							+ result.size() + " favourites!");
