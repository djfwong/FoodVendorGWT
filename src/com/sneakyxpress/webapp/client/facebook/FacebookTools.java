        ...
        try {
            Request response = builder.sendRequest("", new RequestCallback() {
                ...
                public void onResponseReceived(Request request, Response response) {
                    logger.log(Level.INFO, response.getText());
                    if (response.getStatusCode() != 200) {
                        module.addMessage(true, "Could not like with facebook! Please ensure you are logged in and you have not previously liked this vendor.");
                        logger.log(Level.SEVERE, "Error liking with facebook. " + response.getText());
                    } else {
                        module.addMessage(false, "Successfully liked with facebook! Thank you.");
                    }
                }
                ...
                public void onError(Request request, Throwable exception) {
                    logger.log(Level.SEVERE, exception.getMessage());
                    module.addMessage(true, "Could not like with facebook. Reason: " + exception.getMessage());
                }
            });
        } catch (RequestException e) {
            logger.log(Level.SEVERE, e.getMessage());
            module.addMessage(true, "Could not like to facebook. Reason: " + e.getMessage());
        }
    }
